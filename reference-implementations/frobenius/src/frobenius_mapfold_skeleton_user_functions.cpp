...
inline double square_uf(double a){
	return a * a;
}

inline double sum_uf(double a, double b){
	return a + b;
}

inline double mapFold(){
	#pragma omp declare reduction(sum : double : omp_out = sum_uf(omp_out, omp_in)) initializer(omp_priv = omp_orig)
	...
	for(size_t counter = 0; counter < 1073741824; ++counter){
		double map_fold_tmp = square_uf(as[counter]);
		fold_result_double = sum_uf(fold_result_double,map_fold_tmp);
	}		
...
int main(int argc, char** argv) {
	...
	if(process_id == 0){
	printf("Run Frobenius_mapfold skeleton user functions\n\n");			
	}
