...
float sum_uf(int row_index, int col_index, float a){
	float sum = a;
				for(int k = 0; k < 8192; k++){
					sum += as[row_index * 8192 + k] * bs[k * 8192 + col_index];
				}
	return sum;
}
	...
	#pragma omp parallel for
		...
		#pragma omp simd
			for(size_t counter_cols = 0; counter_cols < 8192; ++counter_cols){				
				cs[counter_rows * 8192 + counter_cols] = sum_uf(counter_rows, counter_cols, cs[counter_rows * 8192 + counter_cols]);				
			}
		}
}
...
int main(int argc, char** argv) {
	...
	if(process_id == 0){
	printf("Run Matmult_float skeleton user function\n\n");			
	}
