...
class SkeletonFusionTransformation extends PreprocessorTransformation {
	...
	override run(Resource input) {
		...
		maps.forEach[
			...
			val wait = mainBlock.eAllContents.filter(SkeletonExpression).dropWhile[it !== map].filter[it.obj === (map.eContainer as MusketAssignment).^var.value]
			val what = wait.hasNext()
			if(wait.hasNext()) {		
