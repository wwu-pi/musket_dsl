...
class CMakeGenerator {
	...
	def static CMakeListstxtContent(Resource resource) '''
		...
		«FOR processId : 0 ..< Config.processes»
			add_executable(«resource.ProjectName»_«processId» ${PROJECT_SOURCE_DIR}/src/«resource.ProjectName»_«processId».cpp)
			target_include_directories(«resource.ProjectName»_«processId» PRIVATE ${PROJECT_SOURCE_DIR}/include/«IF Config.processes > 1» ${MPI_CXX_INCLUDE_PATH}«ENDIF»)
			target_compile_options(«resource.ProjectName»_«processId» PRIVATE«IF Config.processes > 1» ${MPI_CXX_COMPILE_FLAGS}«ENDIF» ${OpenMP_CXX_FLAGS})
			target_link_libraries(«resource.ProjectName»_«processId» PRIVATE«IF Config.processes > 1» ${MPI_CXX_LINK_FLAGS} ${MPI_CXX_LIBRARIES}«ENDIF» ${OpenMP_CXX_FLAGS})
		«ENDFOR»
	'''
}
