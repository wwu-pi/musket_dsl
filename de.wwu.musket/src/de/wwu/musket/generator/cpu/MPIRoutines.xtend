...
class MPIRoutines {
	...
	def static generateMPIIsend(int source, String send_buffer, int count, String type, String target, String request) '''
		«Config.tmp_size_t» = «count» * sizeof(«type»);
	...
	def static generateMPIIrecv(int target, String recv_buffer, int count, String type, String source, String request) '''
		«Config.tmp_size_t» = «count» * sizeof(«type»);
		int tag_«counter» = ((«source» + «target») * («source» + «target» + 1)) / 2 + «target»;
