...
class MPIRoutines {
	...
	def static generateMPIGather(String send_buffer, long count, String type, String recv_buffer) '''
	...
	def static generateMPIAllgather(String send_buffer, long count, String type, String recv_buffer) '''
		«Config.tmp_size_t» = «count» * sizeof(«type»);
		MPI_Allgather(«send_buffer», «Config.tmp_size_t», MPI_BYTE, «recv_buffer», «Config.tmp_size_t», MPI_BYTE, MPI_COMM_WORLD);
	'''

	def static generateMPIIsend(int source, String send_buffer, long count, String type, int target, String request) '''
		«Config.tmp_size_t» = «count» * sizeof(«type»);
	...
	def static generateMPIIrecv(int target, String recv_buffer, long count, String type, int source, String request) '''
		«Config.tmp_size_t» = «count» * sizeof(«type»);
		«val tag = ((source + target) * (source + target + 1)) / 2 + target»
		MPI_Irecv(«recv_buffer», «Config.tmp_size_t», MPI_BYTE, «source», «tag», MPI_COMM_WORLD, «request»);
	'''

	def static generateMPIIsend(int source, String send_buffer, long count, String type, String target,
		String request) '''
		«Config.tmp_size_t» = «count» * sizeof(«type»);
	...
	def static generateMPIIrecv(int target, String recv_buffer, long count, String type, String source,
		String request) '''
		«Config.tmp_size_t» = «count» * sizeof(«type»);
		int tag_«counter» = ((«source» + «target») * («source» + «target» + 1)) / 2 + «target»;
