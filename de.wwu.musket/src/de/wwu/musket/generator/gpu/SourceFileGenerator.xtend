...
import static de.wwu.musket.generator.gpu.ShiftSkeletonGenerator.generateShiftSkeletonVariables
import static de.wwu.musket.generator.gpu.MusketFunctionCalls.generateAllTimerGlobalVars
import static extension de.wwu.musket.generator.gpu.MPIRoutines.*
		...
		for (skeletonExpression : resource.SkeletonExpressions.reject[it.skeleton.param.toFunction === null]) {
			...
			if (!(Config.processes < 2 &&
				...
				if (!generated.contains(skelContainerName -> func.name)) {
					...
					result +=
						FunctorGenerator.generateFunctor(func, skel.skeletonName.toString, colname,
			...
			if (!generated.contains(skelContainerName -> mapFunction.name)) {
				...
				result +=
					FunctorGenerator.generateFunctor(mapFunction, skel.skeletonName.toString,
						colname, skeletonExpression.getNumberOfFreeParameters(mapFunction), processId)
			}
		}
		return result
	}
