...
class RunScriptGenerator {
	...
	def static BuildAndRunScriptContent(Resource resource) '''
		...
		cmake -G "Unix Makefiles" -D CMAKE_BUILD_TYPE=«IF Config.mode == Mode.DEBUG»Dev«ELSE»Test«ENDIF» -D CMAKE_CXX_COMPILER=pgc++«IF Config.processes > 1» -D MPI_CXX_COMPILER=/opt/pgi/linux86-64/18.10/mpi/openmpi/bin/mpic++«ENDIF» ../ && \
