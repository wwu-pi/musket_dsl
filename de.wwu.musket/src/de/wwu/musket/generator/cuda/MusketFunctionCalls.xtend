...
class MusketFunctionCalls {
	...
	def static generatePow(MusketFunctionCall mfc, int processId) {
		...
		if(mfc.inFunction)
			return '''__powf(«base.generateExpression(null, processId)», «exponent.generateExpression(null, processId)»)'''
		else{
	...
	def static generateRand(MusketFunctionCall mfc, int processId){
		...
		if(mfc.inFunction)
			if(lower.calculateType.type == PrimitiveTypeLiteral.FLOAT)
				return '''(curand_uniform(&curand_state) * («higher.generateExpression(null, processId)» - «lower.generateExpression(null, processId)») + «lower.generateExpression(null, processId)»)'''
			else if(lower.calculateType.type == PrimitiveTypeLiteral.DOUBLE){
				return '''static_cast<«cpptype»>(curand_uniform(&curand_state) * («higher.generateExpression(null, processId)» - «lower.generateExpression(null, processId)») + «lower.generateExpression(null, processId)»)'''
			}else{ //int
				return '''static_cast<«cpptype»>(truncf(curand_uniform(&curand_state) * («higher.generateExpression(null, processId)» - «lower.generateExpression(null, processId)» + 0.9999999) + «lower.generateExpression(null, processId)»))'''
			}
		else{
			return '''''' // only allowed in funtion --> should be in validator
		}
	}
