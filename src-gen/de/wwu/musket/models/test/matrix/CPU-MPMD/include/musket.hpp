...
void print_dist_ads(const mkt::DMatrix<int>& m, const MPI_Datatype& dt);
void print_dist_bds(const mkt::DMatrix<int>& m, const MPI_Datatype& dt);
...
void mkt::print_dist_ads(const mkt::DMatrix<int>& m, const MPI_Datatype& dt) {
  std::array<int, 16> m_copy;
  MPI_Gatherv(m.get_data(), 4, MPI_INT, m_copy.data(), (std::array<int, 4>{1, 1, 1, 1}).data(), (std::array<int, 4>{0, 1, 4, 5}).data(), dt, 0, MPI_COMM_WORLD);
  std::ostringstream stream;
  stream << "ads" << ": " << std::endl;
  for (int i = 0; i < m.get_number_of_rows(); ++i) {
    stream << "[";
    for (int j = 0; j < m.get_number_of_columns() - 1; ++j) {
      mkt::print<int>(stream, m_copy[i * m.get_number_of_columns() + j]);
      stream << "; ";
    }
    mkt::print<int>(stream, m_copy[i * m.get_number_of_columns() + m.get_number_of_columns() - 1]);
    stream << "]" << std::endl;
  }		  
  stream << std::endl;
  printf("%s", stream.str().c_str());
}
void mkt::print_dist_bds(const mkt::DMatrix<int>& m, const MPI_Datatype& dt) {
  std::array<int, 16> m_copy;
  MPI_Gatherv(m.get_data(), 4, MPI_INT, m_copy.data(), (std::array<int, 4>{1, 1, 1, 1}).data(), (std::array<int, 4>{0, 1, 4, 5}).data(), dt, 0, MPI_COMM_WORLD);
  std::ostringstream stream;
  stream << "bds" << ": " << std::endl;
...
template<>
void mkt::gather<int>(const mkt::DMatrix<int>& in, mkt::DMatrix<int>& out, const MPI_Datatype& dt){
	MPI_Allgatherv(in.get_data(), 4, MPI_INT, out.get_data(), (std::array<int, 4>{1, 1, 1, 1}).data(), (std::array<int, 4>{0, 1, 4, 5}).data(), dt, MPI_COMM_WORLD);
}
