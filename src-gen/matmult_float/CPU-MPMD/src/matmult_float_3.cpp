...
#include <memory>
#include <cstddef>
#include "../include/matmult_float_3.hpp"
...
int main(int argc, char** argv) {
	...
	if(shift_target != 3){
		...
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_37, MPI_COMM_WORLD, &requests[0]);
	...
	if(shift_target != 3){
		...
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_39, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	for(int i = 0; ((i) < 2); ++i){
		...
		if(shift_target != 3){
			...
			MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_41, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
		...
		if(shift_target != 3){
			...
			MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_43, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 3){
		...
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_45, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 3){
		...
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_47, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
