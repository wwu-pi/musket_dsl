...
int main(int argc, char** argv) {
	...
	if(shift_target != 0){
		...
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_1, MPI_COMM_WORLD, &requests[0]);
	...
	if(shift_target != 0){
		...
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_3, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	for(int i = 0; ((i) < 2); ++i){
		...
		if(shift_target != 0){
			...
			MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_5, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
		...
		if(shift_target != 0){
			...
			MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_7, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 0){
		...
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_9, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 0){
		...
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_11, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
