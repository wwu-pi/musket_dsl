	...
	#include "../include/matmult_float_2.cuh"
	...
	const int dim = 4;
	mkt::DMatrix<float> as(2, 4, 4, 2, 2, 16, 4, 1.0f, 2, 2, 1, 0, 2, 0, mkt::DIST, mkt::DIST);
	mkt::DMatrix<float> bs(2, 4, 4, 2, 2, 16, 4, 0.001f, 2, 2, 1, 0, 2, 0, mkt::DIST, mkt::COPY);
	mkt::DMatrix<float> cs(2, 4, 4, 2, 2, 16, 4, 0.0f, 2, 2, 1, 0, 2, 0, mkt::DIST, mkt::DIST);
	...
	int main(int argc, char** argv) {
		...
		// show matrix dist
		as.update_self();
		MPI_Gatherv(as.get_data(), 4, MPI_FLOAT, nullptr, nullptr, nullptr, nullptr, 0, MPI_COMM_WORLD);
		MPI_Barrier(MPI_COMM_WORLD);
		// show matrix dist
		bs.update_self();
		MPI_Gatherv(bs.get_data(), 4, MPI_FLOAT, nullptr, nullptr, nullptr, nullptr, 0, MPI_COMM_WORLD);
		MPI_Barrier(MPI_COMM_WORLD);
		mkt::sync_streams();
		...
		// show matrix dist
		cs.update_self();
		MPI_Gatherv(cs.get_data(), 4, MPI_FLOAT, nullptr, nullptr, nullptr, nullptr, 0, MPI_COMM_WORLD);
		MPI_Barrier(MPI_COMM_WORLD);
