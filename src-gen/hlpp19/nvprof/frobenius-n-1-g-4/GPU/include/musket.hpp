...
void mkt::map(const mkt::DMatrix<T>& in, mkt::DMatrix<R>& out, Functor f) {
	...
	for(int gpu = 0; gpu < 4; ++gpu){
		...
		#pragma acc parallel loop deviceptr(in_devptr, out_devptr) firstprivate(f) async(0) gang vector
...
void mkt::map_in_place(mkt::DMatrix<T>& m, Functor f) {
	...
	for(int gpu = 0; gpu < 4; ++gpu){
		...
		#pragma acc parallel loop deviceptr(devptr) firstprivate(f) async(0) gang vector
		for(unsigned int i = 0; i < gpu_elements; ++i) {
			f.set_id(__pgi_gangidx(), __pgi_workeridx(),__pgi_vectoridx());
			f(devptr[i]);
		}
...
void mkt::map_local_index_in_place(mkt::DMatrix<T>& m, Functor f){
	...
	for(int gpu = 0; gpu < 4; ++gpu){
		...
		#pragma acc parallel loop deviceptr(devptr) firstprivate(f) async(0) gang vector collapse(2)
