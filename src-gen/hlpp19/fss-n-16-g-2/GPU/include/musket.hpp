...
// Getter and Setter
 ...
 private:
  ...
  std::vector<T> _data;
  std::array<T*, 2> _host_data;
  std::array<T*, 2> _gpu_data;
};
...
mkt::DArray<T>::DArray(int pid, int size, int size_local, T init_value, int partitions, int partition_pos, int offset, Distribution d, Distribution device_dist)
    ...
    if(device_dist == mkt::Distribution::DIST){
    	_size_gpu = size_local / 2; // assume even distribution for now
...
template<>
void mkt::print<Fish>(std::ostringstream& stream, const Fish& a) {
  stream << "[";
  stream << "position: [";
  for(int i = 0; i < 511; ++i){
    mkt::print<double>(stream, a.position[i]);
    stream << ", ";
  }
  mkt::print<double>(stream, a.position[511]);
  stream << "]";stream << "; ";
  stream << "fitness: ";
  mkt::print<double>(stream, a.fitness);stream << "; ";
  stream << "candidate_position: [";
  for(int i = 0; i < 511; ++i){
    mkt::print<double>(stream, a.candidate_position[i]);
    stream << ", ";
  }
  mkt::print<double>(stream, a.candidate_position[511]);
  stream << "]";stream << "; ";
  stream << "candidate_fitness: ";
  mkt::print<double>(stream, a.candidate_fitness);stream << "; ";
  stream << "displacement: [";
  for(int i = 0; i < 511; ++i){
    mkt::print<double>(stream, a.displacement[i]);
    stream << ", ";
  }
  mkt::print<double>(stream, a.displacement[511]);
  stream << "]";stream << "; ";
  stream << "fitness_variation: ";
  mkt::print<double>(stream, a.fitness_variation);stream << "; ";
  stream << "weight: ";
  mkt::print<double>(stream, a.weight);stream << "; ";
  stream << "best_position: [";
  for(int i = 0; i < 511; ++i){
    mkt::print<double>(stream, a.best_position[i]);
    stream << ", ";
  }
  mkt::print<double>(stream, a.best_position[511]);
  stream << "]";stream << "; ";
  stream << "best_fitness: ";
  mkt::print<double>(stream, a.best_fitness);
  stream << "]";
}
...
template<>
void mkt::gather<Fish>(mkt::DArray<Fish>& in, mkt::DArray<Fish>& out){
	in.update_self();
	MPI_Allgather(in.get_data(), 128, Fish_mpi_type, out.get_data(), 128, Fish_mpi_type, MPI_COMM_WORLD);
	out.update_devices();
}
template<>
void mkt::gather<double>(mkt::DArray<double>& in, mkt::DArray<double>& out){
	in.update_self();
	MPI_Allgather(in.get_data(), 512, MPI_DOUBLE, out.get_data(), 512, MPI_DOUBLE, MPI_COMM_WORLD);
	out.update_devices();
}
template<typename T>
