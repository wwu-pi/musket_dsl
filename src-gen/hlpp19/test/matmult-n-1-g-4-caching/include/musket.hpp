...
// Getter and Setter
  ...
  unsigned int get_index(int local_row, int local_column) const;
...
mkt::DMatrix<T>::DMatrix(int pid, int number_of_rows, int number_of_columns, int number_of_rows_local, 
	...
	if(device_dist == mkt::Distribution::DIST){
    	...
    	_rows_gpu = number_of_rows_local / 4;
...
void mkt::map(const mkt::DMatrix<T>& in, mkt::DMatrix<R>& out, Functor f) {
	...
	for(int gpu = 0; gpu < 4; ++gpu){
		...
		#pragma acc parallel loop deviceptr(in_devptr, out_devptr) firstprivate(f) async(0) vector_length(1024)
...
template<typename T>
unsigned int mkt::DeviceMatrix<T>::get_index(int local_row, int local_column) const {
	unsigned int device_row = local_row - _device_row_offset;
	unsigned int device_column = local_column - _device_column_offset;
	return device_row * _columns_device + device_column;
}
