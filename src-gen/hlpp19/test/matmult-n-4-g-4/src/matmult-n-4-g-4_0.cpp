	...
	const int dim = 16384;
	mkt::DMatrix<float> as(0, 16384, 16384, 8192, 8192, 268435456, 67108864, 1.0f, 2, 2, 0, 0, 0, 0, mkt::DIST, mkt::DIST);
	mkt::DMatrix<float> bs(0, 16384, 16384, 8192, 8192, 268435456, 67108864, 0.001f, 2, 2, 0, 0, 0, 0, mkt::DIST, mkt::COPY);
	mkt::DMatrix<float> cs(0, 16384, 16384, 8192, 8192, 268435456, 67108864, 0.0f, 2, 2, 0, 0, 0, 0, mkt::DIST, mkt::DIST);
	...
	int main(int argc, char** argv) {
		...
		wait_all_gpus();
		std::chrono::high_resolution_clock::time_point shift_timer_start = std::chrono::high_resolution_clock::now();
		...
		std::chrono::high_resolution_clock::time_point shift_timer_end = std::chrono::high_resolution_clock::now();

		shift_time += std::chrono::duration<double>(shift_timer_end - shift_timer_start).count();
		...
		for(int i = 0; ((i) < 2); ++i){
			...
			shift_timer_start = std::chrono::high_resolution_clock::now();
			...
			wait_all_gpus();
			shift_timer_end = std::chrono::high_resolution_clock::now();
			shift_time += std::chrono::duration<double>(shift_timer_end - shift_timer_start).count();
		}
