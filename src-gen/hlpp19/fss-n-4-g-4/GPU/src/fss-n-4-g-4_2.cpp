	...
	std::array<double,512> mkt::map_reduce_plus<Fish, std::array<double,512>, Lambda38_map_reduce_array_functor>(mkt::DArray<Fish>& a, Lambda38_map_reduce_array_functor f){
			...
			for(int gpu = 0; gpu < 4; ++gpu){
				...
				for (int counter = 0; counter < 512; ++counter) {
					...
					for (int inner_counter = 0; inner_counter < gpu_elements; ++inner_counter) {
						double map_result = (f(devptr[counter]))[inner_counter]; // this is actually calculate more often than necessary
		...
		if(a.get_distribution() == mkt::Distribution::DIST){
			MPI_Allreduce(&local_result.data(), &global_result.data(), 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
			return global_result;
		}else if(a.get_distribution() == mkt::Distribution::COPY){
			return local_result;
		}				
