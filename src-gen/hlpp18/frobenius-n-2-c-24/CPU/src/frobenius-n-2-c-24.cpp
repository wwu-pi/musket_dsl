...
#include <limits>
#include <memory>
#include "../include/frobenius-n-2-c-24.hpp"
...
std::vector<double> as(536870912);
...
int main(int argc, char** argv) {
	...
	for(size_t counter = 0; counter  < 536870912; ++counter){
		as[counter] = 0;
	}
	...
	for(size_t counter_rows = 0; counter_rows < 32768; ++counter_rows){
		...
		for(size_t counter_cols = 0; counter_cols < 32768; ++counter_cols){
			...
			as[counter_rows * 32768 + counter_cols] = ((static_cast<double>(((row_offset + counter_rows))) + ((col_offset + counter_cols))) + 1);
		}
	}
