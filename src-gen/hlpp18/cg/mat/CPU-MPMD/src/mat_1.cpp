...
int main(int argc, char** argv) {
	...
	if(shift_target != 1){
		...
		MPI_Isend(as.data(), 64, MPI_FLOAT, shift_target, tag_13, MPI_COMM_WORLD, &requests[0]);
	...
	if(shift_target != 1){
		...
		MPI_Isend(bs.data(), 64, MPI_FLOAT, shift_target, tag_15, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	for(int i = 0; ((i) < 2); ++i){
		...
		if(shift_target != 1){
			...
			MPI_Isend(as.data(), 64, MPI_FLOAT, shift_target, tag_17, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
		...
		if(shift_target != 1){
			...
			MPI_Isend(bs.data(), 64, MPI_FLOAT, shift_target, tag_19, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 1){
		...
		MPI_Isend(as.data(), 64, MPI_FLOAT, shift_target, tag_21, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 1){
		...
		MPI_Isend(bs.data(), 64, MPI_FLOAT, shift_target, tag_23, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
