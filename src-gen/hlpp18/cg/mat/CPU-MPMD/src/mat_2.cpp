...
int main(int argc, char** argv) {
	...
	if(shift_target != 2){
		...
		MPI_Isend(as.data(), 1024, MPI_FLOAT, shift_target, tag_25, MPI_COMM_WORLD, &requests[0]);
	...
	if(shift_target != 2){
		...
		MPI_Isend(bs.data(), 1024, MPI_FLOAT, shift_target, tag_27, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	for(int i = 0; ((i) < 2); ++i){
		...
		if(shift_target != 2){
			...
			MPI_Isend(as.data(), 1024, MPI_FLOAT, shift_target, tag_29, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
		...
		if(shift_target != 2){
			...
			MPI_Isend(bs.data(), 1024, MPI_FLOAT, shift_target, tag_31, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 2){
		...
		MPI_Isend(as.data(), 1024, MPI_FLOAT, shift_target, tag_33, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 2){
		...
		MPI_Isend(bs.data(), 1024, MPI_FLOAT, shift_target, tag_35, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
