...
std::vector<float> as(64);
std::vector<float> bs(64);
std::vector<float> cs(64);
...
int main(int argc, char** argv) {
	...
	for(size_t counter = 0; counter  < 64; ++counter){
		as[counter] = 1.0f;
	}
	...
	for(size_t counter_rows = 0; counter_rows < 8; ++counter_rows){
		...
		for(size_t counter_cols = 0; counter_cols < 8; ++counter_cols){
			...
			as[counter_rows * 8 + counter_cols] = ((static_cast<float>(((row_offset + counter_rows))) * 4) + ((col_offset + counter_cols)));
		}
	}
	switch(process_id){
	case 0: {
	...
	for(size_t counter_rows = 0; counter_rows < 8; ++counter_rows){
		...
		for(size_t counter_cols = 0; counter_cols < 8; ++counter_cols){
			...
			bs[counter_rows * 8 + counter_cols] = ((static_cast<float>(16) + (((row_offset + counter_rows)) * 4)) + ((col_offset + counter_cols)));
		}
	}
	...
	for(int i = 0; ((i) < 2); ++i){
		...
		for(size_t counter_rows = 0; counter_rows < 8; ++counter_rows){
			...
			for(size_t counter_cols = 0; counter_cols < 8; ++counter_cols){
				...
				float sum = (cs[counter_rows * 8 + counter_cols]);
				for(int k = 0; ((k) < 8); k++){
					sum += ((as)[(counter_rows) * 8 + (k)] * (bs)[(k) * 8 + (counter_cols)]);
				}
				cs[counter_rows * 8 + counter_cols] = (sum);
			}
		}
		switch(process_id){
		case 0:	{
