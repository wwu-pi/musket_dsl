...
std::chrono::high_resolution_clock::time_point gather_start;
std::chrono::high_resolution_clock::time_point gather_end;
double gather_elapsed = 0.0;
size_t tmp_size_t = 0;
...
int main(int argc, char** argv) {
	...
	MPI_Allgather(P.data(), 125000, Particle_mpi_type, oldP.data(), 125000, Particle_mpi_type, MPI_COMM_WORLD);
	...
	for(int i = 0; ((i) < (steps)); ++i){
		...
		MPI_Allgather(P.data(), 125000, Particle_mpi_type, oldP.data(), 125000, Particle_mpi_type, MPI_COMM_WORLD);
		gather_end = std::chrono::high_resolution_clock::now();
		gather_elapsed += std::chrono::duration<double>(gather_end - gather_start).count();
	}
	std::chrono::high_resolution_clock::time_point timer_end = std::chrono::high_resolution_clock::now();
	double seconds = std::chrono::duration<double>(timer_end - timer_start).count();
	printf("Elapsed time in seconds for timer %s: %.5f\n", "map", map_elapsed);
	printf("Elapsed time in seconds for timer %s: %.5f\n", "gather", gather_elapsed);
