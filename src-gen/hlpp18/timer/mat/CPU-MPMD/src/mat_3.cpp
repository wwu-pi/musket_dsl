...
int main(int argc, char** argv) {
	...
	if(shift_target != 3){
		...
		int tag_84 = ((shift_source + 3) * (shift_source + 3 + 1)) / 2 + 3;
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_84, MPI_COMM_WORLD, &requests[1]);
		int tag_85 = ((3 + shift_target) * (3 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_85, MPI_COMM_WORLD, &requests[0]);
	...
	if(shift_target != 3){
		...
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_86, MPI_COMM_WORLD, &requests[1]);
		int tag_87 = ((3 + shift_target) * (3 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_87, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	for(int i = 0; ((i) < 2); ++i){
		...
		if(shift_target != 3){
			...
			auto tmp_shift_buffer = std::make_unique<std::vector<float>>(67108864);
			tmp_size_t = 67108864 * sizeof(float);
			int tag_88 = ((shift_source + 3) * (shift_source + 3 + 1)) / 2 + 3;
			MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_88, MPI_COMM_WORLD, &requests[1]);
			int tag_89 = ((3 + shift_target) * (3 + shift_target + 1)) / 2 + shift_target;
			MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_89, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
		...
		if(shift_target != 3){
			...
			auto tmp_shift_buffer = std::make_unique<std::vector<float>>(67108864);
			tmp_size_t = 67108864 * sizeof(float);
			int tag_90 = ((shift_source + 3) * (shift_source + 3 + 1)) / 2 + 3;
			MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_90, MPI_COMM_WORLD, &requests[1]);
			int tag_91 = ((3 + shift_target) * (3 + shift_target + 1)) / 2 + shift_target;
			MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_91, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 3){
		...
		auto tmp_shift_buffer = std::make_unique<std::vector<float>>(67108864);
		tmp_size_t = 67108864 * sizeof(float);
		int tag_92 = ((shift_source + 3) * (shift_source + 3 + 1)) / 2 + 3;
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_92, MPI_COMM_WORLD, &requests[1]);
		int tag_93 = ((3 + shift_target) * (3 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_93, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 3){
		...
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_94, MPI_COMM_WORLD, &requests[1]);
		int tag_95 = ((3 + shift_target) * (3 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_95, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
