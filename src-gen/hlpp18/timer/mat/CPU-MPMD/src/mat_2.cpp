...
int main(int argc, char** argv) {
	...
	if(shift_target != 2){
		...
		int tag_72 = ((shift_source + 2) * (shift_source + 2 + 1)) / 2 + 2;
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_72, MPI_COMM_WORLD, &requests[1]);
		int tag_73 = ((2 + shift_target) * (2 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_73, MPI_COMM_WORLD, &requests[0]);
	...
	if(shift_target != 2){
		...
		auto tmp_shift_buffer = std::make_unique<std::vector<float>>(67108864);
		tmp_size_t = 67108864 * sizeof(float);
		int tag_74 = ((shift_source + 2) * (shift_source + 2 + 1)) / 2 + 2;
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_74, MPI_COMM_WORLD, &requests[1]);
		int tag_75 = ((2 + shift_target) * (2 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_75, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	for(int i = 0; ((i) < 2); ++i){
		...
		if(shift_target != 2){
			...
			MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_76, MPI_COMM_WORLD, &requests[1]);
			int tag_77 = ((2 + shift_target) * (2 + shift_target + 1)) / 2 + shift_target;
			MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_77, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
		...
		if(shift_target != 2){
			...
			auto tmp_shift_buffer = std::make_unique<std::vector<float>>(67108864);
			tmp_size_t = 67108864 * sizeof(float);
			int tag_78 = ((shift_source + 2) * (shift_source + 2 + 1)) / 2 + 2;
			MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_78, MPI_COMM_WORLD, &requests[1]);
			int tag_79 = ((2 + shift_target) * (2 + shift_target + 1)) / 2 + shift_target;
			MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_79, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 2){
		...
		auto tmp_shift_buffer = std::make_unique<std::vector<float>>(67108864);
		tmp_size_t = 67108864 * sizeof(float);
		int tag_80 = ((shift_source + 2) * (shift_source + 2 + 1)) / 2 + 2;
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_80, MPI_COMM_WORLD, &requests[1]);
		int tag_81 = ((2 + shift_target) * (2 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_81, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 2){
		...
		auto tmp_shift_buffer = std::make_unique<std::vector<float>>(67108864);
		tmp_size_t = 67108864 * sizeof(float);
		int tag_82 = ((shift_source + 2) * (shift_source + 2 + 1)) / 2 + 2;
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_82, MPI_COMM_WORLD, &requests[1]);
		int tag_83 = ((2 + shift_target) * (2 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_83, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
