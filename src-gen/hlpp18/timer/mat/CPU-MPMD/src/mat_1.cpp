...
int main(int argc, char** argv) {
	...
	if(shift_target != 1){
		...
		int tag_60 = ((shift_source + 1) * (shift_source + 1 + 1)) / 2 + 1;
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_60, MPI_COMM_WORLD, &requests[1]);
		int tag_61 = ((1 + shift_target) * (1 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_61, MPI_COMM_WORLD, &requests[0]);
	...
	if(shift_target != 1){
		...
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_62, MPI_COMM_WORLD, &requests[1]);
		int tag_63 = ((1 + shift_target) * (1 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_63, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	for(int i = 0; ((i) < 2); ++i){
		...
		if(shift_target != 1){
			...
			auto tmp_shift_buffer = std::make_unique<std::vector<float>>(67108864);
			tmp_size_t = 67108864 * sizeof(float);
			int tag_64 = ((shift_source + 1) * (shift_source + 1 + 1)) / 2 + 1;
			MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_64, MPI_COMM_WORLD, &requests[1]);
			int tag_65 = ((1 + shift_target) * (1 + shift_target + 1)) / 2 + shift_target;
			MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_65, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
		...
		if(shift_target != 1){
			...
			MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_66, MPI_COMM_WORLD, &requests[1]);
			int tag_67 = ((1 + shift_target) * (1 + shift_target + 1)) / 2 + shift_target;
			MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_67, MPI_COMM_WORLD, &requests[0]);
			MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 1){
		...
		auto tmp_shift_buffer = std::make_unique<std::vector<float>>(67108864);
		tmp_size_t = 67108864 * sizeof(float);
		int tag_68 = ((shift_source + 1) * (shift_source + 1 + 1)) / 2 + 1;
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_68, MPI_COMM_WORLD, &requests[1]);
		int tag_69 = ((1 + shift_target) * (1 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(as.data(), 67108864, MPI_FLOAT, shift_target, tag_69, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
	...
	if(shift_target != 1){
		...
		MPI_Irecv(tmp_shift_buffer->data(), tmp_size_t, MPI_FLOAT, shift_source, tag_70, MPI_COMM_WORLD, &requests[1]);
		int tag_71 = ((1 + shift_target) * (1 + shift_target + 1)) / 2 + shift_target;
		MPI_Isend(bs.data(), 67108864, MPI_FLOAT, shift_target, tag_71, MPI_COMM_WORLD, &requests[0]);
		MPI_Waitall(2, requests, statuses);
