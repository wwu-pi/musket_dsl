...
std::chrono::high_resolution_clock::time_point fold_start;
std::chrono::high_resolution_clock::time_point fold_end;
double fold_elapsed = 0.0;
size_t tmp_size_t = 0;
...
int main(int argc, char** argv) {
	...
	for(size_t counter_rows = 0; counter_rows < 16384; ++counter_rows){
		...
		for(size_t counter_cols = 0; counter_cols < 16384; ++counter_cols){
			size_t counter = counter_rows * 16384 + counter_cols;
	...
	fold_elapsed = 0.0;
	fold_start = std::chrono::high_resolution_clock::now();
	double fn = 0.0;
	...
	fold_end = std::chrono::high_resolution_clock::now();
	fold_elapsed += std::chrono::duration<double>(fold_end - fold_start).count();
	fn = std::sqrt((fn));
