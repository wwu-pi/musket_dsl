...
std::chrono::high_resolution_clock::time_point fitness_start;
std::chrono::high_resolution_clock::time_point fitness_end;
double fitness_elapsed = 0.0;
std::chrono::high_resolution_clock::time_point individual_start;
std::chrono::high_resolution_clock::time_point individual_end;
double individual_elapsed = 0.0;
std::chrono::high_resolution_clock::time_point feeding_start;
std::chrono::high_resolution_clock::time_point feeding_end;
double feeding_elapsed = 0.0;
std::chrono::high_resolution_clock::time_point instinctive_start;
std::chrono::high_resolution_clock::time_point instinctive_end;
double instinctive_elapsed = 0.0;
std::chrono::high_resolution_clock::time_point volitive_start;
std::chrono::high_resolution_clock::time_point volitive_end;
double volitive_elapsed = 0.0;
std::chrono::high_resolution_clock::time_point solution_start;
std::chrono::high_resolution_clock::time_point solution_end;
double solution_elapsed = 0.0;
size_t tmp_size_t = 0;
...
std::vector<Fish> population(512);
std::vector<double> instinctive_movement_vector_copy(512);
std::vector<Fish> weighted_fishes(512);
...
struct Lambda9_functor{
	auto operator()(Fish fi) const{
		return (fi).fitness_variation;
	}
};
struct Lambda10_functor{
	auto operator()(Fish fi) const{
		return (fi).displacement;
	}
};
struct Lambda11_functor{
	auto operator()(Fish fi) const{
		return (fi).weight;
	}
};
struct Lambda12_functor{
	auto operator()(Fish fi) const{
		return (fi).position;
	}
};
struct Lambda13_functor{
	auto operator()(Fish fi) const{
...
int main(int argc, char** argv) {
	...
	Lambda9_functor lambda9_functor{};
	Lambda10_functor lambda10_functor{};
	Lambda11_functor lambda11_functor{};
	Lambda12_functor lambda12_functor{};
	Lambda13_functor lambda13_functor{};
	...
	for(size_t counter = 0; counter < 512; ++counter){
		double map_fold_tmp = lambda7_functor(population[counter]);
	...
	init_elapsed += std::chrono::duration<double>(init_end - init_start).count();
	for(int iteration = 0; ((iteration) < (ITERATIONS)); ++iteration){
		fitness_start = std::chrono::high_resolution_clock::now();
		#pragma omp parallel for simd
		for(size_t counter = 0; counter < 512; ++counter){
			population[counter] = evaluateFitness_functor(population[counter]);
		}
		fitness_end = std::chrono::high_resolution_clock::now();
		fitness_elapsed += std::chrono::duration<double>(fitness_end - fitness_start).count();
		if(((iteration) > 0)){
		...
		individual_start = std::chrono::high_resolution_clock::now();
		#pragma omp parallel for simd
		for(size_t counter = 0; counter < 512; ++counter){
			population[counter] = individualMovement_functor((step_size), population[counter]);
		}
		individual_end = std::chrono::high_resolution_clock::now();
		individual_elapsed += std::chrono::duration<double>(individual_end - individual_start).count();
		feeding_start = std::chrono::high_resolution_clock::now();
		...
		for(size_t counter = 0; counter < 512; ++counter){
			double map_fold_tmp = lambda8_functor(population[counter]);
		...
		feeding_end = std::chrono::high_resolution_clock::now();
		feeding_elapsed += std::chrono::duration<double>(feeding_end - feeding_start).count();
		instinctive_start = std::chrono::high_resolution_clock::now();
		...
		for(size_t counter = 0; counter < 512; ++counter){
			double map_fold_tmp = lambda9_functor(population[counter]);
		...
		for(size_t counter = 0; counter < 512; ++counter){
			std::array<double,512> map_fold_tmp = lambda10_functor(population[counter]);
		...
		instinctive_end = std::chrono::high_resolution_clock::now();
		instinctive_elapsed += std::chrono::duration<double>(instinctive_end - instinctive_start).count();
		volitive_start = std::chrono::high_resolution_clock::now();
		...
		#pragma omp parallel for simd reduction(sumWeight_reduction:fold_result_double)
		for(size_t counter = 0; counter < 512; ++counter){
			double map_fold_tmp = lambda11_functor(population[counter]);
		...
		for(size_t counter = 0; counter < 512; ++counter){
			std::array<double,512> map_fold_tmp = lambda12_functor(weighted_fishes[counter]);
		...
		volitive_end = std::chrono::high_resolution_clock::now();
		volitive_elapsed += std::chrono::duration<double>(volitive_end - volitive_start).count();
	}
	solution_elapsed = 0.0;
	solution_start = std::chrono::high_resolution_clock::now();
	...
	for(size_t counter = 0; counter < 512; ++counter){
		double map_fold_tmp = lambda13_functor(population[counter]);
	...
	solution_end = std::chrono::high_resolution_clock::now();
	solution_elapsed += std::chrono::duration<double>(solution_end - solution_start).count();
	std::chrono::high_resolution_clock::time_point timer_end = std::chrono::high_resolution_clock::now();
	double seconds = std::chrono::duration<double>(timer_end - timer_start).count();
	printf("Best solution: %.5f\n",(global_best_fitness));
	printf("Elapsed time in seconds for timer %s: %.5f\n", "init", init_elapsed);
	printf("Elapsed time in seconds for timer %s: %.5f\n", "fitness", fitness_elapsed);
	printf("Elapsed time in seconds for timer %s: %.5f\n", "individual", individual_elapsed);
	printf("Elapsed time in seconds for timer %s: %.5f\n", "feeding", feeding_elapsed);
	printf("Elapsed time in seconds for timer %s: %.5f\n", "instinctive", instinctive_elapsed);
	printf("Elapsed time in seconds for timer %s: %.5f\n", "volitive", volitive_elapsed);
	printf("Elapsed time in seconds for timer %s: %.5f\n", "solution", solution_elapsed);
