...
#include <limits>
#include <memory>
#include "../include/fss-n-2-c-18.hpp"
...
std::vector<Fish> population(1024);
std::vector<double> instinctive_movement_vector_copy(512);
std::vector<Fish> weighted_fishes(1024);
...
int main(int argc, char** argv) {
	...
	if(process_id == 0){
	printf("Run Fss-n-2-c-18\n\n");			
	}
	
	
	std::vector<std::mt19937> random_engines;
	random_engines.reserve(18);
	std::random_device rd;
	for(size_t counter = 0; counter < 18; ++counter){
	...
	std::vector<std::uniform_real_distribution<double>> rand_dist_double_INIT_LOWER_BOUND_INIT_UPPER_BOUND;
							rand_dist_double_INIT_LOWER_BOUND_INIT_UPPER_BOUND.reserve(18);
							for(size_t counter = 0; counter < 18; ++counter){
							...
							rand_dist_double_minus1_0_1_0.reserve(18);
							for(size_t counter = 0; counter < 18; ++counter){
							...
							rand_dist_double_0_0_1_0.reserve(18);
							for(size_t counter = 0; counter < 18; ++counter){
	...
	#pragma omp declare reduction(calcDisplacementFold : std::vector<double> : omp_out = [&](){for(int i = 0; ((i) < (DIMENSIONS)); ++i){	omp_out[(i)] += (omp_in)[(i)];}return (omp_out);}()) initializer(omp_priv = omp_orig)
	#pragma omp declare reduction(calcBarycenterFold : std::vector<double> : omp_out = [&](){for(int i = 0; ((i) < (DIMENSIONS)); ++i){	omp_out[(i)] += (omp_in)[(i)];}return (omp_out);}()) initializer(omp_priv = omp_orig)
	...
	double fold_result_double;std::vector<double> fold_result_std_vector_double_;
	...
	for(int iteration = 0; ((iteration) < (ITERATIONS)); ++iteration){
		...
		fold_result_std_vector_double_.assign(512, 0.0);
		
		
		#pragma omp parallel for simd reduction(calcDisplacementFold:fold_result_std_vector_double_)
		for(size_t counter = 0; counter < 1024; ++counter){
			std::vector<double> map_fold_tmp;
			
			map_fold_tmp = (population[counter]).displacement;
		
			
			for(int i = 0; ((i) < (DIMENSIONS)); ++i){
				fold_result_std_vector_double_[(i)] += (map_fold_tmp)[(i)];
			}
		}		
		
		MPI_Allreduce(fold_result_std_vector_double_.data(), instinctive_movement_vector_copy.data(), 512 * sizeof(double), MPI_BYTE, calcDisplacementFold_mpi_op, MPI_COMM_WORLD); 
		#pragma omp parallel for simd
		for(size_t counter = 0; counter < 512; ++counter){
		...
		#pragma omp parallel for simd reduction(calcBarycenterFold:fold_result_std_vector_double_)
		for(size_t counter = 0; counter < 1024; ++counter){
			std::vector<double> map_fold_tmp;
		...
		MPI_Allreduce(fold_result_std_vector_double_.data(), barycenter_copy.data(), 512 * sizeof(double), MPI_BYTE, calcBarycenterFold_mpi_op, MPI_COMM_WORLD); 
		#pragma omp parallel for simd
		for(size_t counter = 0; counter < 512; ++counter){
