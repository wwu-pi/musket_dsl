...
#include <limits>
#include <memory>
#include "../include/fss-n-4-c-1.hpp"
...
int main(int argc, char** argv) {
	...
	#pragma omp simd reduction(sumWeight:fold_result_double)
	for(size_t counter = 0; counter < 1024; ++counter){
		double map_fold_tmp;
	...
	for(int iteration = 0; ((iteration) < (ITERATIONS)); ++iteration){
		...
		#pragma omp simd reduction(maxFitnessVariation:fold_result_double)
		for(size_t counter = 0; counter < 1024; ++counter){
			double map_fold_tmp;
		...
		#pragma omp simd reduction(sumFitnessVariation:fold_result_double)
		for(size_t counter = 0; counter < 1024; ++counter){
			double map_fold_tmp;
		...
		#pragma omp simd reduction(calcDisplacementFold:fold_result_std_array_double_0_)
		for(size_t counter = 0; counter < 1024; ++counter){
		...
		#pragma omp simd reduction(calcBarycenterFold:fold_result_std_array_double_0_)
		for(size_t counter = 0; counter < 1024; ++counter){
			std::array<double,0> map_fold_tmp;
	...
	#pragma omp simd reduction(getBestSolution:fold_result_double)
	for(size_t counter = 0; counter < 1024; ++counter){
		double map_fold_tmp;
