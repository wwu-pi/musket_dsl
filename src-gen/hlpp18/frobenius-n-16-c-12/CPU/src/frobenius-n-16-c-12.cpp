...
#include <limits>
#include <memory>
#include "../include/frobenius-n-16-c-12.hpp"
...
int main(int argc, char** argv) {
	...
	if(process_id == 0){
	printf("Run Frobenius-n-16-c-12\n\n");			
	}
	...
	for(size_t counter_rows = 0; counter_rows < 8192; ++counter_rows){
		...
		for(size_t counter_cols = 0; counter_cols < 8192; ++counter_cols){
			...
			as[counter_rows * 8192 + counter_cols] = ((static_cast<double>(((row_offset + counter_rows))) + ((col_offset + counter_cols))) + 1);
		}
	}
